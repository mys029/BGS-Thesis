...
rw_mcmc <- function(grid){
  ...
  while(continue){
    ...
    x <- runif(1,0,1)
    if(x < proposed_score/current_score){
      current_grid <- grid
    }
  ...
  return(c(final_score, start_score, data.frame(score_vector)))
...
```{r}
x1 <- data.frame(iter = rep(1:10000), scores = x[[3]])
ggplot(aes(x = iter, y = scores), data = x1) +
  geom_line()
```
