...
```{r}
database_new2 <- database_new %>% select(-c(pool,score))
```

```{r, eval=FALSE}
first_row <- score_grid(matrix(c(t(database_new2[1,])),nrow=5,ncol=4,byrow=T), individual=TRUE)
...
for(i in 2:nrow(database_new2)){
  row <- as.list(score_grid(matrix(c(t(database_new2[i,])),nrow=5,ncol=4,byrow=T), individual=TRUE))
  db_individual <- rbind(db_individual, row)
}
```

```{r, eval=FALSE}
# write.csv(db_individual, here::here("db_individual.csv"), row.names = FALSE)
```

```{r}
db_individual <- read.csv(here::here("db_individual.csv"))
```

```{r}
individual_matrix <- as.matrix(db_individual)
```
