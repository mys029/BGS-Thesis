...
```{r}
rw_mcmc <- function(grid, iterations = 1000, acceptance_func = "simple", beta = 0, bp = 500, original = NULL){
...
```{r}
multi_mcmc <- function(iterations, n, acceptance_func = "simple", beta = 0, bp = 500, grid = NULL){
  start_scores <- c()
  highest_scores <- c()
  highest_iter <- c()
  cards <- c(rep("Bear", 12), 
...
```{r}
tune <- function(iter, beta, bp, type){
  beta_df <- c()
  bp_df <- c()
  iter_df <- c()
  start_score <- c()
...
```{r, eval=FALSE}
beta <- c(0.85, 0.9, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98, 0.99)
bp <- c(125)
iter <- c(500)

set.seed(49)
tune1 <- tune(iter, beta, bp, "annealing dynamic")
```
...
```{r, eval=FALSE}
# write.csv(tune1,here::here("tuned-parameters.csv"))
```
...
```{r, eval=FALSE}
new_params <- rbind(tuned_params, tune1)
write.csv(new_params,here::here("tuned-parameters.csv"))
```
