...
```{r}
rw_mcmc <- function(grid, iterations = 1000, acceptance_func = "simple", beta = 0, bp = 500){
  ...
  highest_grid <- grid
  while(continue){
    current_score <- solo_score(score_grid(grid))
    if(iter%%bp == 0){
    ...
    while(grid[start_row, start_col] == grid[end_row, end_col]){
      end_row <- sample(rows, 1)
      end_col <- sample(cols, 1)
    }
    ...
    grid[start_row, start_col] = end
    grid[end_row, end_col] = start
    
    proposed_score <- solo_score(score_grid(grid))
  ...
  final_score <- solo_score(score_grid(grid))
  if(final_score > highest_score){
      highest_score = final_score
      highest_grid = grid
    }
  
  return(c(highest_score, final_score, start_score, data.frame(score_vector)))
  
}
```

```{r}
multi_mcmc <- function(iterations, n, acceptance_func = "simple", beta = 0, bp = 500){
  start_scores <- c()
  highest_scores <- c()
  cards <- c(rep("Bear", 12), 
           rep("Bee", 8), 
  ...
  return(list(start_scores, highest_scores))
}
```
...
```{r}
set.seed(4)
x2 <- multi_mcmc(2000, 100, "annealing", 0.08)
```
...
```{r}
set.seed(4)
x3 <- multi_mcmc(2000, 100, "annealing dynamic", 0.05)
```
...
```{r}
set.seed(4)
x4 <- multi_mcmc(4000, 100, "delayed", 0.005, 250)
```
...
```{r}
set.seed(4)
x5 <- multi_mcmc(4000, 100, "annealing dynamic", 0.005, 250)
```
...
```{r}
set.seed(4)
x6 <- multi_mcmc(4000, 100, "delayed", 0.01, 500)
```
...
x <- rw_mcmc(sim_grid1, 2000, "annealing dynamic", beta = 0.05, 500)
```
