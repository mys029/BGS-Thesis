...
first_row <- score_grid(matrix(c(t(database_new3[1,])),nrow=5,ncol=4,byrow=T), individual=TRUE)
...
for(i in 2:nrow(database_new3)){
  row <- as.list(score_grid(matrix(c(t(database_new3[i,])),nrow=5,ncol=4,byrow=T), individual=TRUE))
  db_individual <- rbind(db_individual, row)
}
```

```{r, eval=FALSE}
# write.csv(db_individual, here::here("db_individual_lowest_removed.csv"), row.names = FALSE)
```
