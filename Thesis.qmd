...
```{r}
multi_mcmc <- function(iterations, n, acceptance_func = "simple", beta = 0, bp = 500, grid = NULL, boardlist = NULL, record_board = FALSE){
  first = TRUE
  start_scores <- c()
  highest_scores <- c()
  highest_iter <- c()
...
```{r, eval=FALSE}
startTime <- Sys.time()
db_gen <- multi_mcmc(1000, 2000, "annealing dynamic", 0.9, 200, record_board = TRUE)
endTime <- Sys.time()
